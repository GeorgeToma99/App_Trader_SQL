...
Select
	...
	name,
	ROUND((10000 * 12 * expected_life) - marketingPrice
	- (CASE WHEN app_price < 1 THEN 10000
	ELSE 10000 * app_price END)) AS total_revenue,
	expected_life,
	app_price,
	avg_rating
...
FROM (
	...
	ROUND((p.rating + a.rating)/2, 1) AS avg_rating,
			-- TO GET THE HIGHER APP PRICE:
		GREATEST(
			a.price, 
	TRIM(REPLACE (p.price ,'$', ''))::NUMERIC) AS app_price,
--	((ROUND((p.rating + a.rating)/2))*2) + 1 AS expected_life
	1 + (2*(ROUND(a.rating + p.rating) / 2)) AS expected_life,
1000 * 12 * ( 1 + ( 2 *( round(a.rating + p.rating) / 2 ) ) ) as marketingPrice
	
	FROM play_store_apps as p
	INNER JOIN app_store_apps as a
	USING(name)
--	WHERE a.price IS NOT NULL AND p.price IS NOT NULL
	ORDER BY expected_life DESC
--GROUP BY a.name, a.price, p.price, a.rating, p.rating -- was made to add this, not sure why
) 
